    ...
    <el-table :data="tableData" stripe row-key="id" default-expand-all>
      ...
      <el-table-column label="操作" width="270">
        <template v-slot="scope">
          ...
          <!--          编辑-->
          <el-button type="success" v-if="!scope.row.pid" @click="handleAdd(scope.row)"><i class="el-icon-folder-add">添加二级分类</i>
          </el-button>
          <el-button type="primary" icon="el-icon-edit"
                     @click="$router.push('/editEatCategory?id=' + scope.row.id)"></el-button>
...
export default {
  ...
  methods: {
    ...
    del(id) {
      request.delete("/eatCategory/delete/" + id).then(res => {
