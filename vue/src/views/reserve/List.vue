  ...
  <div>
    ...
    <el-table :data="tableData" stripe row-key="id" default-expand-all>
      ...
      <el-table-column label="管理">
        <template v-slot="scope">
          <el-button type="primary" @click="returnHotels(scope.row)" v-if="scope.row.status === '已售出'">退房</el-button>
        </template>
      </el-table-column>
...
export default {
  ...
  methods: {
    ...
    returnHotels(row) {
      request.post("/reserve/saveRetur", row).then(res => {
        if (res.code === '200') {
